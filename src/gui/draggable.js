  ...
  draggable(element,funcStart,funcOver,funcEnd) {
    ...
    const dragstart = (event) => {
      ...
      const moveAt = (pageX, pageY) => {
        ...
        DRAG.newDX = (pageX  - DRAG.orgX); ///TWIN.zoom; // WRONG
        DRAG.newDY = (pageY  - DRAG.orgY); ///TWIN.zoom;
      }
...
export  const edgeEnd = (event) => {
    ...
    TWIN.graph.appendEdge(DRAG.edge,getID(event.target.id) );
  ...
  export const dragStartNode = (event) => {
    ...
    // Update Edges
    TWIN.graph.updateEdges(dragged,isShrinked);
    return dragged;
  }
  ...
  export const dragOverNode = (event) => {
    ...
    *************/
    ...
    DRAG.node.style.left = ((DRAG.BBox.x - TWIN.tx)/TWIN.zoom + DRAG.newDX/TWIN.zoom)  + 'px';
    DRAG.node.style.top  = ((DRAG.BBox.y - TWIN.ty)/TWIN.zoom + DRAG.newDY/TWIN.zoom)  + 'px';
  ...
  export const dragEndNode = (event) => {
    ...
    TWIN.graph.updateEdges(dragged,dragged.classList.contains('shrink'));
  }
...
export const translOver = (event) => {
  ...
  document.querySelector('#board').style.transform = `
    translate(50%,50%) 
    scale(${TWIN.zoom}) 
    translate(${TWIN.translate.x + DRAG.newDX}px,${TWIN.translate.y + DRAG.newDY}px) 
    translate(-50%,-50%)`;
  TWIN.graph.updateAllEdges(document.querySelectorAll('section'));
...
export const translEnd = (event) => {
  ...
  TWIN.translate.x += DRAG.newDX;
  TWIN.translate.y += DRAG.newDY;
  event.preventDefault();
}
